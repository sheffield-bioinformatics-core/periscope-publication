...
add_stat_smooth <- function(data, mapping, ...){
  p <- ggplot(data = data, mapping = mapping) + 
    ...
    stat_cor(geom="text",size=3,method="pearson",fill='#FFFFFF',color='#4D4845',alpha=0.3,vjust=0.92,label.padding=unit(0.2, "lines"),aes(label=..rr.label..))+
                stat_cor(geom="text",size=3,method="pearson",fill='#FFFFFF',color='#4D4845',alpha=0.3,vjust=0.92,label.padding=unit(0.2, "lines"),aes(label=..p..*6),vjust=1.1)+
    theme(
...
a = ggplot(sg%>%filter(orf!="ORF1a")%>%filter(orf!="ORF10")%>%filter(orf!="N*"),aes(reorder(orf,-gRNA_count,median),gRNA_count))+
...
b = ggplot(sg%>%filter(orf!="ORF1a")%>%filter(orf!="ORF10")%>%filter(orf!="N*")%>%filter(orf!="ORF10"),aes(reorder(orf,-gRHPT,median),gRHPT))+
  geom_boxplot(color=navy)+
  ft_theme()+
...
m=melt(sg%>%filter(orf!="N*")%>%filter(orf!="ORF1a")%>%filter(!is.na(sgRPTg_HQ))%>%select(sample,orf,sgRPTg_HQ,sgRPTg_LQ))
...
scale_x_continuous("sgRPTg")+
scale_y_continuous("sgRPTg")+
...
c = ggpairs(spread_down
  ...
  ft_theme()+
  scale_x_continuous("sgRPTg (Log10)",trans=scales::pseudo_log_trans(base = 10),breaks=c(0,10,100,1000,10000))+
...
row2 = ggarrange(d,e,g,h,labels=c("C","D","E","F"),ncol=4,nrow=1)
row3= ggarrange(ggmatrix_gtable(c),labels=c("G"))
figure = ggarrange(row1,row2,row3,nrow=3,ncol=1,heights=c(0.5,0.4,1))
figure
...
```{r supp_figure_s13_ect_coverage , fig.width=6, fig.height=4, echo=F,warnings=F,message=F}
setwd("/home/md1mpar/Documents/projects/periscope/genome_research_revision/summary_data")
ct = read.csv("supplementary_file_9_sample_ect.csv")
sg = read.csv("supplementary_file_1_cannonical_orf_counts.csv")
...
#### Supplementary Figure SX - Read-Level evidence for non-canonical sub-genomic RNA at 5785 and 10639


```{r , fig.width=8.25, fig.height=7, echo=F,warnings=F,message=F}
